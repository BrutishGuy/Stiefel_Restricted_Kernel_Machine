...
class Net1(nn.Module):
    ...
    def __init__(self, nChannels, args, cnn_kwargs):
        ...
        self.main = nn.Sequential(
            ...
            nn.Conv2d(self.args.capacity, self.args.capacity, kernel_size=3, padding=1),
            nn.GELU(),

            nn.Conv2d(self.args.capacity, self.args.capacity * 2, **cnn_kwargs[0]),
            nn.GELU(),

            nn.Conv2d(self.args.capacity * 2, self.args.capacity * 2, kernel_size=3, padding=1),
            nn.GELU(),
...
class Net3(nn.Module):
    ...
    def __init__(self, nChannels, args, cnn_kwargs):
        ...
        self.main = nn.Sequential(
            ...
            nn.ConvTranspose2d(self.args.capacity * 4, self.args.capacity * 2, output_padding=1, **cnn_kwargs[1]),
            nn.GELU(),
            nn.Conv2d(self.args.capacity * 2, self.args.capacity * 2, kernel_size=3, padding=1),
            nn.GELU(),
            nn.ConvTranspose2d(self.args.capacity * 2, self.args.capacity, output_padding=1, **cnn_kwargs[0]),
            nn.GELU(),
            nn.Conv2d(self.args.capacity, self.args.capacity, kernel_size=3, padding=1),
            nn.GELU(),
            nn.ConvTranspose2d(self.args.capacity, nChannels, output_padding=1, **cnn_kwargs[0]),
            nn.Tanh()
        )
...
class RKM_Stiefel(nn.Module):
    ...
    def __init__(self, ipVec_dim, args, nChannels=1, recon_loss=nn.MSELoss(reduction='sum'), ngpus=1):
        ...
        self.cnn_kwargs = dict(kernel_size=3, stride=2, padding=1)
        if self.ipVec_dim <= 28*28*3:
            ...
            elif self.args.dataset_name == 'imagenette':
