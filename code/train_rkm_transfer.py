...
from stiefel_rkm_model_pretrained import *
import logging
import argparse
import time
...
x_transfer_features = get_transfer_features(args=opt)
...
while cost > 1e-10 and t <= opt.max_epochs:  # run epochs until convergence or cut-off
    ...
    for indices, sample_batched in enumerate(tqdm(xtrain, desc="Epoch {}/{}".format(t, opt.max_epochs))):
        ...
        x_transfer_feature_batch = x_transfer_features[x_transfer_features.Image == sample_indices]
        x_transfer_feature_batch = torch.tensor(x_transfer_feature_batch.drop(['Image'], axis = 1).values.astype(np.float32))
        
        loss, f1, f2 = rkm(x_transfer_feature_batch.to(device), sample_batched[0].to(device))
